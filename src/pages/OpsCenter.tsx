...
import {
  ...
  type SLOSummaryData,
  type RetrievalQualityData,
  type IngestionAlertsData,
} from "@/lib/api";
import {
  Activity, TrendingUp, TrendingDown, AlertTriangle, Bell,
  CheckCircle, Clock, Server, Zap, BarChart3, Ticket,
  Rocket, Webhook, Plus, RefreshCw, Eye, Loader2
} from "lucide-react";
...
const OpsCenter = () => {
  ...
  const createTicketMutation = useMutation({
    mutationFn: () => opsApi.createTicket({ title: formTitle, severity: formPriority, description: formDesc }),
    onSuccess: () => {
  ...
  return (
    <AppLayout>
              ...
              <div className="bg-card rounded-lg border border-border shadow-xs overflow-hidden overflow-x-auto">
                <table className="w-full text-sm min-w-[700px]">
                  ...
                  <tbody className="divide-y divide-border">
                      ...
                      <tr key={t.ticket_id} className="hover:bg-secondary/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${
                            ...
                            t.status === "investigating" ? "bg-warning/10 text-warning" :
                    ...
                    {tickets.length === 0 && (
                      <tr><td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">暂无工单</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className={`ml-2 text-[11px] px-2 py-0.5 rounded-full font-medium ${
                            ...
                            r.status === "rolling" ? "bg-info/10 text-info" :
      ...
      {/* New Incident */}
      <FormDialog open={showNewIncident} onClose={() => { setShowNewIncident(false); setFormTitle(""); setFormDesc(""); }} title="创建工单"
        footer={<>
        ...
        <FormField label="严重级别" required>
          <FormSelect value={formPriority} onChange={setFormPriority} options={[
            { value: "critical", label: "紧急" }, { value: "high", label: "高" }, { value: "medium", label: "中" }, { value: "low", label: "低" },
          ]} />
        </FormField>
